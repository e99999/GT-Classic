...
public class XSTR extends Random {
	...
	// rivate static final int PROBE_INCREMENT = 0x9e3779b9;
	 ...
	 *
	 * <p>
	 * The hedge "approx imately" is used in the foregoing description only because
	...
	public int nextInt(int bound) {
		...
		// }
		/*
		 * int r = next(31); int m = bound - 1; if ((bound & m) == 0) // i.e., bound is
		 * a power of 2 { r = (int) ((bound * (long) r) >> 31); } else { for (int u = r;
		 * u - (r = u % bound) + m < 0; u = next(31)) ; } return r;
		 */
	...
	public void nextBytes(byte[] bytes_arr) {
		for (int iba = 0, lenba = bytes_arr.length; iba < lenba;)
			for (int rndba = nextInt(), nba = Math.min(lenba - iba, Integer.SIZE
					/ Byte.SIZE); nba-- > 0; rndba >>= Byte.SIZE)
