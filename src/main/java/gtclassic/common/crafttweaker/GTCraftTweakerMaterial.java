...
public class GTCraftTweakerMaterial {
	...
	public GTCraftTweakerMaterial(String displayName, int r, int g, int b, String[] flags) {
		if (GTMaterial.hasMaterial(displayName)) {
			CraftTweakerAPI.logError(CraftTweakerAPI.getScriptFileAndLine() + " > Material " + displayName
					+ " already Exists!");
			return;
		}
		if (flags.length == 0) {
	...
	public static GTCraftTweakerMaterial getMaterial(String name) {
		if (GTMaterial.hasMaterial(name)) {
			...
			CraftTweakerAPI.logError(CraftTweakerAPI.getScriptFileAndLine() + " > Material " + name
					+ " does not Exist!");
			return null;
		}
	}
	...
	public static void createFlag(String suffix, String domain, String path, boolean layered, boolean block) {
		if (GTMaterialFlag.getMapSize() == 64) {
			CraftTweakerAPI.logError(CraftTweakerAPI.getScriptFileAndLine()
					+ " > Max count of Material Flags already reached!");
			return;
		}
		String prefix = suffix.replace("_", "");
		if (GTMaterialFlag.hasFlag(prefix)) {
			CraftTweakerAPI.logError(CraftTweakerAPI.getScriptFileAndLine() + " > Material Flag " + suffix
					+ " already Exists!");
			return;
		}
		...
		GTMaterialFlag flag = new GTMaterialFlag(suffix, domain + "_"
				+ prefix, 0, layered, domain).setCraftweaker(true);
		if (block) {
