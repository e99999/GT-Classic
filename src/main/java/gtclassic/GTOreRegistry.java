...
public enum GTOreRegistry {
	...
	BISMUTHTINE(1, GTMaterial.Bismuthtine, Blocks.SAND, 8, 8, 50, 80),
	CALCITE(2, GTMaterial.Calcite, 8, 12, 20, 80), 
	CASSITERITE(3, GTMaterial.Cassiterite, Blocks.SAND, 32, 2, 60, 120),
	CHROMITE(4, GTMaterial.Chromite, 8, 4, 0, 20), 
	...
	MAGNETITE(13, GTMaterial.Magnetite, Blocks.SAND, 32, 1, 60, 120),
	...
	SALTPETER(19, GTMaterial.Saltpeter, 16, 1, 0, 80),
	...
	TUNGSTATE(26, GTMaterial.Tungstate, 4, 4, 0, 20), 
	BASALT(27, GTMaterial.Basalt, Blocks.GRAVEL, 24, 2, 16, 50),
	...
	VIBRANIUM(31, GTMaterial.Vibranium, 2, 1, 0, 128);
	...
	GTOreRegistry(int id, GTMaterial mat, Block type, int size, int chance, int min, int max) {
		this.mat = mat;
		this.id = id;
		this.type = type;
		this.size = size;
		...
		for (Block block : Block.REGISTRY) {
			...
			if (block instanceof GTBlockOreStone) {
				...
				if (ore.getOreEntry().equals(OLIVINE)) {
					GameRegistry.addSmelting(block, GTMaterialGen.getGem(GTMaterial.Olivine, 3), 0.1F);
				}

				if (ore.getOreEntry().equals(RUBY)) {
					GameRegistry.addSmelting(block, GTMaterialGen.getGem(GTMaterial.Ruby, 3), 0.1F);
				}
				...
				if (ore.getOreEntry().equals(SPHALERITE)) {
					GameRegistry.addSmelting(block, GTMaterialGen.getNugget(GTMaterial.Lead, 3), 0.1F);
				}
			}
